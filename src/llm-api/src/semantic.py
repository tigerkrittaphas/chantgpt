...
def _resolve_data_dir() -> Path:
    ...
    for parent in [start, *start.parents]:
        candidate = parent / "src" / "llm-api" / "data" / "processed"
        if (candidate / "pali_dictionary_with_thai.csv").exists():
            return candidate
    return start.parents[1] / "data" / "processed"


DATA_DIR = _resolve_data_dir()
DICTIONARY_FILE_PATH = DATA_DIR / "pali_dictionary_with_thai.csv"
INDEX_PATH = DATA_DIR / "definition_faiss.index"
METADATA_PATH = DATA_DIR / "definition_faiss_meta.json"
...
def _resolve_vertex(project: str | None, location: str | None) -> Tuple[str, str]:
    ...
    region = location or os.getenv("VERTEX_LOCATION") or os.getenv("GOOGLE_LOCATION") or "us-central1"
    if not project_id:
